  ...
  >
    <div class="relative mb-4 rounded-md overflow-hidden aspect-[9/16]">
      <img
        ...
        :src="$imageService(blok.image.filename, '405x720')"
        :srcset="`
          ${$imageService(blok.image.filename, '259x460')} 259w,
          ${$imageService(blok.image.filename, '362x644')} 362w,
          ${$imageService(blok.image.filename, '405x720')} 405w
        `"
    ...
    >
      ...
      <span class="text-beige-100 text-sm font-semibold">{{ channel.name }}</span>
    </div>
  </a>
</template>
